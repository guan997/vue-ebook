  ...
  <div class="search-bar-wrapper">
    ...
    >
      ...
      <div
        ...
        :class="{ 'show-search': ifShowSearchPage }"
        @click="back"
      >
    ...
    <transition name="host-search">
      <div
        ...
        v-if="ifShowSearchPage && ifShowHotSearch"
        ref="searchMaskWrapper"
      >
        <!-- 热门搜索 -->
...
const BOOK_Search_KEY = 'search'
  export default {
    ...
    methods: {
      setKeyword(keyword) {
        // 判断缓存是否有记录 如果有historySearch更新
        if ( getLocalStorage(BOOK_Search_KEY)) {
          this.searchList.historySearch = getLocalStorage(BOOK_Search_KEY)
        }
        this.searchText = keyword
        let length = this.searchList.historySearch.length;
        // 添加记录
        this.searchList.historySearch[length]={
          text:keyword,
          type:1,
          id:length
        }
        setLocalStorage(BOOK_Search_KEY, this.searchList.historySearch)
      },
      search() {
        this.$router.push({
      ...
      hideShadow() {
        ...
        // if ( !this.searchList.historySearch ) {
        //   this.searchList.historySearch = getLocalStorage(BOOK_Search_KEY)
        // }
      },
      ...
      hideSearchPage() {this.searchText = ''
